...
public:
    ...
    void AddDocument(int document_id, const std::string& document, DocumentStatus status, const std::vector<int>& ratings) {
        ...
        if (document_id < 0) {
            throw std::invalid_argument("document id less than zero");
        }
        else if (documents_.find(document_id) != documents_.end()) {
            throw std::invalid_argument("document already added");
        }
...
private:
    ...
    QueryWord ParseQueryWord(std::string text) const {
        ...
        if (text[0] == '-') {
            throw std::invalid_argument("query contains -");
        }
        else if (text.size() == 0) {
             throw std::invalid_argument("query is empty");
        }
        else if (!IsValidWord(text)) {
             throw std::invalid_argument("query contains forbidden symbols)");
        }
    ...
    double ComputeWordInverseDocumentFreq(const std::string& word) const {
        double wordfreqsize = static_cast<double>(word_to_document_freqs_.at(word).size());
        assert (wordfreqsize != 0);
        assert(word_to_document_freqs_.count(word) > 0);

        return log(GetDocumentCount() * 1.0 / wordfreqsize);
    }
...
const double SearchServer::double_tolerance = 1e-6;
